...
function UserPhotos(props) {
  ...
  const axios_post_handle = (url, info, errMsg) => {
    axios.post(url, info) // send new like action to server
      .then(() => axios_fetch_photos(props.match.params.userId))  // use axios to get user's photos, traggering the page updating.
  ...
  // * Meaning: if ther is login user or there is no user or no photos
  if (props.loginUser || !user || !photos) {

    /* If there is user and user's photo, then display user photos */
    return (user && (
